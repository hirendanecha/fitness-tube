...
export class AddCommunityModalComponent implements OnInit, AfterViewInit {
  ...
  userId = '';
  profileId = '';
  originUrl = environment.webUrl + 'fitness-trainers/';
  logoImg: any = {
    file: null,
    url: '',
  };
        ...
        this.communityService.createCommunity(formData).subscribe({
          next: (res: any) => {
            ...
            if (!res.error) {
              ...
              this.toastService.success(
                'Your Fitness Trainers will be approved within 24 hours!'
              );
          ...
          error: (err) => {
            this.toastService.danger(
              'Please change trainer. this trainer name already in use.'
            );
            this.spinner.hide();
          },
        });
        ...
        .subscribe({
          next: (res: any) => {
            ...
            if (!res.error) {
              ...
              // this.createCommunityAdmin(res.data);
              this.toastService.success(
                'Your Fitness Trainers edit successfully!'
              );
              this.activeModal.close('success');
            }
          },
          error: (err) => {
            this.toastService.danger(
              'Please change Fitness Trainers. this Fitness Trainers name already in use.'
            );
            this.spinner.hide();
          },
        });
